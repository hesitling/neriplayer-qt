...
Defines the `ViewModelError` Q_GADGET and `ErrorType` enum for exposing API, network, database, and validation errors to QML. Provides factory methods to map from `ApiError` and classify retryability.
